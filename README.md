...
**A small tool surface is easier to secure — and easier to test.** Every capability is one small, independent command rather than a sprawling do-everything API, which means each one can be reasoned about — and attacked — in isolation. That composability turns out to double as a testing methodology: `wackypub` can orchestrate a coordinator-and-worker swarm of its own agents to red-team a target tool's actual live build, not a hypothetical. This isn't a design aspiration - there's a real, dogfooded protocol for it ([`docs/SWARM_TESTING.md`](docs/SWARM_TESTING.md)), a tracked pass/fail record per tool ([`.agents/SECURITY_TESTING.md`](.agents/SECURITY_TESTING.md)), and genuine findings from actually running it (a cross-agent file-read gap in a companion tool, caught and documented, not swept under the rug). It's also a legitimate standalone use case for `wackypub` itself, distinct from roleplay or general orchestration - point the swarm at anything with a CLI surface, including `wackypub`'s own companion tools.
