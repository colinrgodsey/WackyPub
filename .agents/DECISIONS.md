...
Workshopped, not yet implemented. New standalone repo (`github.com/colinrgodsey/wackyproc`), vendored the same way as `files-rw` (D50-era extraction): a git submodule at `tools/wackyproc`, source and CI live there, shared process (security testing methodology, decision history) stays in wackypub's own `.agents/`.
